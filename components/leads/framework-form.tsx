// components/leads/framework-form.tsx
"use client";
...
export function FrameworkForm({ source }: Props) {
    ...
    return (
      ...
      >
        ...
        <p className="text-base leading-relaxed text-bone/70">
          The framework is on its way to <span className="text-bone">{email}</span>.
        ...
        {submit.data?.downloadUrl && (
          <p className="text-sm text-bone/50">
            ...
            . Free to share, free to use, no strings.
          </p>
        )}
      </div>
    );
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div style={HONEYPOT_STYLE} aria-hidden="true">
        <label htmlFor="website-fw">Website (leave blank)</label>
        <input
          id="website-fw"
          name="website"
          type="text"
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {submit.isPending ? "Sending…" : (
            <>
              Email me the framework <ArrowRight className="h-4 w-4" />
            </>
          )}
        </button>
        {submit.error && (
      ...
      <p className="text-xs text-bone/40">
        We email you the PDF. No drip sequences, no list rentals, easy
        unsubscribe. Used only to send you what you asked for.
      </p>
    </form>
  );
}
